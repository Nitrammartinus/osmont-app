...
const TimeVariance: React.FC<{ variance: number | null }> = ({ variance }) => {
    ...
    if (variance > 0) {
        return <span className="font-semibold text-red-600">{hours}h nad rámec</span>;
    }
    if (variance < 0) {
        return <span className="font-semibold text-green-600">{hours}h pod rámec</span>;
    }
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <InfoCard icon={<DollarSign />} label="Náklady / Hodina (celkovo)" value={new Intl.NumberFormat('sk-SK', { style: 'currency', currency: 'EUR' }).format(projectData.costPerHour)} />
...
const EvaluationDashboard: React.FC = () => {
    ...
    const filteredEvaluationData = useMemo(() => {
        ...
        for (const project of evaluationSource) {
            const filteredSessions = filteredCompletedSessions.filter(s => s.project_id === project.id);
        ...
        return filteredData;
    }, [projectEvaluation, filteredCompletedSessions, startDate, endDate]);
        ...
        <div className="max-w-7xl mx-auto">
            ...
            {filteredEvaluationData.length === 0 && (
                 <div className="text-center py-16 bg-white rounded-2xl shadow-xl">
                    ...
                    <p className="text-gray-500 mt-2">Pre zvolený časový rozsah neexistuje žiadna aktivita sledovania času.</p>
                </div>
            )}
        </div>
    );
