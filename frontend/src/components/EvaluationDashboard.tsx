...
const ProjectDetailsView: React.FC<{ projectData: ProjectEvaluationData; onBack: () => void; }> = ({ projectData, onBack }) => {
    const exportProjectSessionsToCSV = () => {
        const headers = ['Dátum', 'Čas', 'Zamestnanec', 'Trvanie (formát)', 'Trvanie (minúty)'];
        const csvContent = [
            headers.join(';'),
            ...projectData.allSessions.map(session =>
                `"${new Date(session.timestamp).toLocaleDateString()}";"${new Date(session.timestamp).toLocaleTimeString()}";"${session.employee_name}";"${formatDuration(session.duration_minutes)}";${session.duration_minutes}`
            )
        ].join('\n');
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <InfoCard icon={<DollarSign />} label="Cena / Hodina (celkovo)" value={`${projectData.costPerHour.toFixed(2)} €`} />
                     {projectData.workProgressPercentage !== null &&
                        <InfoCard icon={<TrendingUp />} label="Priebeh (podľa hodín)" value={`${projectData.workProgressPercentage.toFixed(0)}%`} />
                    }
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Rozpis hodín podľa používateľa (v období)</h3>
...
// FIX: Update icon prop type to allow className for React.cloneElement.
        ...
        <div className="max-w-7xl mx-auto">
            ...
            {filteredEvaluationData.length === 0 && (
                 <div className="text-center py-16 bg-white rounded-2xl shadow-xl">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700">Žiadne dáta pre projekty</h3>
