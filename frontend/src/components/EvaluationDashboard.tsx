...
import { Clock, Users, Calendar, DollarSign, Download, BarChart3, TrendingUp, ArrowLeftRight } from './Icons';
...
// FIX: Update InfoCard's icon prop type to allow passing a className.
...
const EvaluationDashboard: React.FC = () => {
    ...
    const filteredEvaluationData = useMemo(() => {
        ...
        evaluationSource.forEach((project: ProjectEvaluationData) => {
    ...
    const totalTrackedTime = filteredSessionsForExport.reduce((sum, s) => sum + s.duration_minutes, 0);

    if (selectedProject) {
        const updatedProjectData = filteredEvaluationData.find(p => p.id === selectedProject.id);
        if(!updatedProjectData) {
            // Project might not have sessions in the filtered range, show its lifetime data instead
            const lifetimeData = projectEvaluation[selectedProject.id];
            return <ProjectDetailsView projectData={lifetimeData} onBack={() => setSelectedProject(null)} />;
        }
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredEvaluationData.map((project) => (
                    <div key={project.id} onClick={() => setSelectedProject(project)} className="bg-white rounded-2xl shadow-lg p-6 cursor-pointer hover:shadow-2xl hover:-translate-y-1 transition-all duration-200 flex flex-col">
                        ...
                        <div className="mt-auto">
                             <div className="flex justify-between items-center text-xs text-gray-600 mb-1">
                                ...
                                <span>{project.workProgressPercentage?.toFixed(0) ?? 'N/A'}%</span>
                            </div>
...
const ProjectDetailsView: React.FC<{ projectData: ProjectEvaluationData; onBack: () => void; }> = ({ projectData, onBack }) => {
     ...
     const exportProjectSessionsToCSV = (sessions: CompletedSession[]) => {
        ...
        const headers = ['Dátum', 'Čas', 'Zamestnanec', 'Trvanie (minúty)', 'Trvanie (formát)'];
        const csvContent = [
            headers.join(';'),
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <button onClick={onBack} className="bg-white px-4 py-2 rounded-lg shadow hover:bg-gray-50 text-sm font-medium">Späť na prehľad</button>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <InfoCard icon={<TrendingUp />} label="Priebeh (podľa hodín)" value={`${projectData.workProgressPercentage?.toFixed(0) ?? 'N/A'}%`} />
                    <InfoCard icon={<ArrowLeftRight />} label="Odchýlka (celkovo)" value={<TimeVariance variance={projectData.timeVariance} />} />
                </div>
            </div>

            <div className="bg-white rounded-2xl shadow-xl p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Čas podľa Používateľa (v období)</h3>
                <div className="space-y-3">
                    {Object.values(projectData.userBreakdown).map((userData: UserBreakdown, index) => (
