...
const UserForm: React.FC<{
    ...
    isEditing: boolean;
}> = ({ user, setUser, allCostCenters, isEditing }) => {
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" placeholder="Celé meno" value={user.name || ''} onChange={e => setUser({ ...user, name: e.target.value })} className="w-full p-2 border rounded" required />
                <input type="text" placeholder="Používateľské meno" value={user.username || ''} onChange={e => setUser({ ...user, username: e.target.value })} className="w-full p-2 border rounded" required />
            </div>
             <input type="password" placeholder={isEditing ? "Nové heslo (nepovinné)" : "Heslo"} onChange={e => setUser({ ...user, password: e.target.value })} className="w-full p-2 border rounded" required={!isEditing} />
            <select value={user.role || 'employee'} onChange={e => setUser({ ...user, role: e.target.value as UserRole })} className="w-full p-2 border rounded">
            ...
            <div className="pt-2">
                <label className="flex items-center space-x-2 text-gray-700">
                    <input type="checkbox" checked={!!user.can_select_project_manually} onChange={e => setUser({ ...user, can_select_project_manually: e.target.checked })} className="rounded" />
                    <span>Môže vyberať projekt manuálne</span>
                </label>
            </div>
            <div>
                <h4 className="font-medium text-gray-700 mb-2">Priradené strediská</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 p-2 border rounded-lg max-h-40 overflow-y-auto">
                    {allCostCenters.map(center => (
                        <label key={center.id} className="flex items-center space-x-2 p-1 rounded hover:bg-gray-100">
                            <input
                                type="checkbox"
        ...
        return (
            <div className="max-w-2xl mx-auto">
                 <div className="bg-white rounded-2xl shadow-xl p-6">
                     ...
                     <UserForm user={editingUser} setUser={(u) => setEditingUser(u as User)} allCostCenters={costCenters} isEditing={true} />
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="bg-white rounded-2xl shadow-xl p-6">
                ...
                <UserForm user={newUser} setUser={setNewUser} allCostCenters={costCenters} isEditing={false} />
